...
class GameActivityLevelHard : AppCompatActivity() {
    var playerSelected = " "

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_game_level_hard)
    ...
    private fun get_appropriate_position(empty_position : List<Int>): Int {
        for (temp in winingPosition ){
            if(
                ...
                (boxContent[temp[0]] == boxContent[temp[2]] )
                and (boxContent[temp[2]]  == 1 )
                and  (boxContent[temp[1]]  == 2 )
            ){
                return temp[1]
            }
            else if(
                (boxContent[temp[1]] == boxContent[temp[2]] )
                and (boxContent[temp[1]]  == 1 )
